...
def Classifier(input_size,output_size):
    input_v=tf.keras.Input( (input_size,) )
    nn=tf.keras.layers.Dense(50)(input_v) #Massive data reduction here !
    nn=tf.keras.layers.LeakyReLU()(nn)
    nn=tf.keras.layers.Dropout(0.8)(nn)

    nn=tf.keras.layers.Dense(50)(nn) #Massive data reduction here !
...
def xgboost(input_size,output_size):
    model = xgb.XGBClassifier(n_esitmators=10,max_depth=3)
    return model
